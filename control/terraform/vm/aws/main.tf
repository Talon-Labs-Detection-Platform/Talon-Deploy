# Talon Control on one EC2 instance: Docker Compose, a private Postgres, and
# Caddy terminating TLS. The same shape docs/production-hosting.md describes by
# hand, expressed as code.
...
resource "aws_security_group" "this" {
  name        = "${var.name}-sg"
  description = "Talon Control: SSH from admins, HTTP/HTTPS from the internet"
  vpc_id      = var.vpc_id
  tags        = local.tags
}
...
resource "aws_instance" "this" {
  ...
  root_block_device {
    ...
    volume_type = "gp3"
    encrypted   = true
    # The volume holds App Logs history, the Claude Code transcripts dev
    # sessions resume from, and git checkouts with uncommitted work in them.
