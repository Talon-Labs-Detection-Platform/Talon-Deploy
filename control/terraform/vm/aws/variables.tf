...
variable "domain" {
  description = <<-EOT
    ...
    must point at the Elastic IP this stack outputs BEFORE the instance first
    boots, or Caddy cannot complete the ACME HTTP-01 challenge and you get no
    certificate. Apply -target=aws_eip.this first, set the record, then apply
    the rest.
  EOT
  type        = string
...
variable "admin_cidrs" {
  description = <<-EOT
    ...
    0.0.0.0/0 on port 22 is how a box ends up in a botnet. Use ["1.2.3.4/32"]
    for a fixed address, or your VPN's range.
  EOT
  type        = list(string)
...
variable "subnet_id" {
  description = "Public subnet within vpc_id. Public because the console is reached from the internet and Caddy needs inbound ACME."
  type        = string
}
...
variable "instance_type" {
  description = <<-EOT
    ...
    the app process, so t3.small will complete them and feel slow; t3.medium is
...
variable "disk_size_gb" {
  description = "Root volume. Holds Postgres, App Logs history, dev-session checkouts and the media library."
  type        = number
  default     = 40
}
