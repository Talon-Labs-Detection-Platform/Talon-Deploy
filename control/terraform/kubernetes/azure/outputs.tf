output "cluster_name" {
  description = "Wire up kubectl: az aks get-credentials --resource-group <rg> --name <this>."
  value       = azurerm_kubernetes_cluster.this.name
}

output "resource_group" {
  description = "Resource group holding the cluster and the database."
...
output "ingress_address" {
  ...
  value       = "kubectl -n ${var.namespace} get ingress ${var.name}-talon-control -o jsonpath='{.status.loadBalancer.ingress[0].ip}'"
}
