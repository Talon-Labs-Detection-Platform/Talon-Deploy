# Talon Studio on one Azure Linux VM: the appliance image (Next.js app, embedded
# Postgres, embedded pySigma) on one /data volume, behind Caddy terminating TLS.
# The AWS stack next door, in Azure's vocabulary.
...
resource "azurerm_linux_virtual_machine" "this" {
  ...
  os_disk {
    ...
    # Holds the embedded Postgres, the git-backed detection repos and the synced
    # detection content — everything the appliance is.
