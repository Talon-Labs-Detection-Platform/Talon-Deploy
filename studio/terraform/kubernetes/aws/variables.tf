...
variable "chart_version" {
  description = <<-EOT
    Version of the published talon-studio chart to install. Tracks the appliance
    release, so pinning it pins both — an empty image_tag below then resolves to
    this chart's own appVersion.
  EOT
  type        = string
  default     = "1.0.0"
}
...
variable "image_tag" {
  description = "Pinned Talon Studio release, e.g. v1.0.0. Empty falls back to the chart's appVersion."
  type        = string
  default     = ""
...
variable "disk_size_gb" {
  description = "The /data PVC. Holds the detection repos, synced content and uploads."
  type        = number
  default     = 40
}
